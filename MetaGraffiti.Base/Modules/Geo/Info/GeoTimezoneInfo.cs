...
namespace MetaGraffiti.Base.Modules.Geo.Info
{
	public class GeoTimezoneInfo
	{
		// ==================================================
		// Internals
		protected GeoTimezoneData _data;


		// ==================================================
		// Constructors
		public GeoTimezoneInfo(GeoTimezoneData data) { _data = data; }
		...
		// --------------------------------------------------
		// Inferred
		public GeoTimezoneTypes TimezoneType => _data.TimezoneType;
		public int GeoTZID => _data.GeoTZID;

		public string Key => _data.Key;
		public string Name => _data.Name;
		public string TZID => _data.TZID;
		public string WinTZ => _data.WinTZ;
		public string DisplayAs => _data.DisplayAs;

		public TimeSpan OffsetUTC => _data.OffsetUTC;
		...
		// --------------------------------------------------
		// Properties
		public static List<GeoTimezoneInfo> All { get { return Cache.ToInfo().ToList(); } }
		...
		//public static List<GeoTimezoneInfo> Defaults { get { return CacheWindows.ToInfo().Where(x => x.IsDefault).ToList(); } }

		public static IEnumerable<GeoTimezoneInfo> Windows { get { return CacheWindows.ToInfo(); } }

		public static IEnumerable<GeoTimezoneInfo> Olson { get { return CacheOlson.ToInfo(); } }

		// --------------------------------------------------
		// Methods
		public static GeoTimezoneInfo BySystem(string winTZ)
		{
			return CacheWindows.FirstOrDefault(x => String.Compare(x.WinTZ, winTZ, true) == 0).ToInfo();
		}

		public static GeoTimezoneInfo ByTZID(string tzid)
		{
			return CacheOlson.FirstOrDefault(x => String.Compare(x.TZID, tzid, true) == 0).ToInfo();
		}

		public static GeoTimezoneInfo ByKey(string key)
		{
			var t = CacheWindows.FirstOrDefault(x => String.Compare(x.Key, key, true) == 0);
			if (t == null) t = CacheOlson.FirstOrDefault(x => String.Compare(x.Key, key, true) == 0);
			return t.ToInfo();
		}

		public static GeoTimezoneInfo ByName(string name)
		{
			var t = CacheWindows.FirstOrDefault(x => String.Compare(x.Name, name, true) == 0);
			if (t == null) t = CacheOlson.FirstOrDefault(x => String.Compare(x.Name, name, true) == 0);
			return t.ToInfo();
		}
		...
		private static Dictionary<int, GeoTimezoneData> _cache = new Dictionary<int, GeoTimezoneData>();
		private static List<GeoTimezoneData> _windows;
		private static List<GeoTimezoneData> _olson;
		private static IEnumerable<GeoTimezoneData> Cache { get { if (!_initialized) Initialize(); return _cache.Values; } }
		private static IEnumerable<GeoTimezoneData> CacheWindows { get { if (!_initialized) Initialize(); return _windows; } }
		private static IEnumerable<GeoTimezoneData> CacheOlson { get { if (!_initialized) Initialize(); return _olson; } }
