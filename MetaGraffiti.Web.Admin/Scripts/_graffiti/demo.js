...
function mapD3() {
	...
	this.drag = false;				// toggles drag mode for globe
	...
	if (points != null) {
		...
		for (var i = 0; i < points.length; i++) {
			...
			if (points[i].attr("cx")) {
				...
				points[i].attr("x", p[0] - 16).attr("y", p[1] - 16);
			}
...
mapD3.prototype.drawPoints = function (places) {
	...
	for (i = 0; i < places.length; i++) {
		...
		point.style("visibility", (visible ? "visible" : "hidden"));
...
mapD3.prototype.drawPlaces = function (places) {
	...
	for (i = 0; i < places.length; i++) {
		var p = _mapD3.d3proj([places[i].lng, places[i].lat]);
		var icon = places[i].type;
		var point = _mapD3.d3g.append("image")
			.on("click", _mapD3.doClickCircle)
			.attr("xlink:href", "/images/places/" + icon + ".svg")
			.attr("width", 32)
			.attr("height", 32)
			.attr("class", "geodot")
			.attr("country", function (d) { return places[i].country; })
